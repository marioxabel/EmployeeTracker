...
export async function addRole() {
    ...
    const { title, salary, department_id } = await inquirer.prompt([
        ...
        },
        {
            ...
            choices: departments.map((element) => element["Department Name"]),
            filter: function (value) {
                const result = departments.find((element) => element["Department Name"] === value);
                return result.ID;
            }
        }
    ]);
...
export async function updateEmployeeRole() {
    ...
    const roles = await getAllRoles();
    const formattedRoles = roles.map((element) => ({
        id: element.ID,
    ...
    const { employee_id, role_id } = await inquirer.prompt([
        {
            ...
            choices: formattedEmployees.map((element) => element.name),
            filter: function (value) {
                const result = formattedEmployees.find((element) => element.name === value);
                return result?.id;
            }
        },
        {
            ...
            choices: formattedRoles.map((element) => element.name),
            filter: function (value) {
                const result = formattedRoles.find((element) => element.name === value);
                return result?.id;
            }
        }
    ]);
...
export async function getEmployeesFromManager() {
    ...
    const employeesWithoutManagers = employees
        .filter((element) => element.Manager === null)
        .map((element) => ({
        id: element.ID,
        name: element['First Name'].concat(' ', element['Last Name']),
    }));
    const { manager } = await inquirer.prompt({
        ...
        choices: employeesWithoutManagers.map((element) => element.name),
        filter: function (value) {
            const result = employeesWithoutManagers.find((element) => element.name === value);
            return result;
        }
    });
    ...
    const { department } = await inquirer.prompt([
        {
            ...
            choices: departments.map((element) => ({
